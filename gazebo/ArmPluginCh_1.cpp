 ...
 * Modified by - Carlos Lacerda, 06/2018
 *
 * Target: The object will instantiate at different locations along the x-axis.
 *         The gripper must touch the tube.
 */
...
/*---------------------------------------------------------------
 * ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /_sdf/)
 * Load model elements
 *---------------------------------------------------------------
 */
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	//WaitUntilSimTime(common::Time(100));
...
bool ArmPlugin::createAgent()
{
	...
	if( !agent )
	{
		printf("ArmPlugin - failed to create DQN agent  %ux%u  %u\n", INPUT_WIDTH, INPUT_HEIGHT, NUM_ACTIONS);
		return false;
	}
...
bool ArmPlugin::updateAgent()
{
	...
	if((action % 2) == 0) action_ADD = true;	// action is even
	
	if(DEBUG){printf("ArmPlugin - agent selected action %i %s\n", action, action_ADD ? "+":"-");}
	...
	*/
	...
	if(action_ADD) velocity += actionVelDelta;
	...
	joint = ref[action/2];
	if(action_ADD) joint += actionJointDelta;
...
/*---------------------------------------------------------------
 * ArmPlugin::updateJoints()
 * Update joint reference positions.
 * Returns: true if positions have been modified
 *---------------------------------------------------------------
 */
...
/*---------------------------------------------------------------
 * BoxDistance(const math::Box& a, const math::Box& b)
...
static void saveAcc(int& episode, float& acc)
{
	if(fptr == NULL) 
		fptr = fopen("arm.txt", "a");
...
/*---------------------------------------------------------------
 * ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		if(gripBBox.max.z <= groundContact) checkGroundContact = true;
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				rewardHistory = avgGoalDelta * multi;
				newReward     = true;
			}
